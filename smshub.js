...
export const orderNum = async () => {
  try {
    ...
    const country = '6';
    const url = `https://smshub.org/stubs/handler_api.php?api_key=${SMSHUB_APIKEY}&action=getNumber&service=${serviceType}&operator=telkomsel&country=${country}&maxPrice=0.0444`;
    const { data } = await axios.get(url);
